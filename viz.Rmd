...
You can do each of these exercises in their own R code chunk.
...
###JAMIE - may want to remove/alter some of these. I don't introduce the concept of "mapping"    
...
To change the geom in your plot, change the geom function that you add to `ggplot()`. For instance, to plot a smoothed mean from the data above, use `geom_smooth`.
...
On the x-axis, the chart displays `trans`, a variable from `mpg`. On the y-axis, it displays count, but count is not a variable in `mpg`! Where does count come from? Many graphs, like scatterplots, plot the raw values of your dataset. Other graphs, like bar charts, calculate new values to plot:
...
Note what happens if you map the fill aesthetic to another variable, like `class`: the bars are automatically stacked. Each colored rectangle represents a combination of `fl` and `class`.
...
## Final save, commit, pull and push to github.
