...
mass <- read_csv("https://raw.githubusercontent.com/OHI-Science/data-science-training/master/data/ma.csv")
```
...
> 4. What's going on here, why isn't the data showing up?
```{r, eval=FALSE, purl=FALSE}
...
ggplot has a special technique called *faceting* that allows the user to split one plot into multiple plots based on a factor included in the dataset. We will use it to make a plot of park visitation by state:
